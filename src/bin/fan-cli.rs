            ...
            let default_pipeline = 
                Pipeline{ 
                    ...
                    operations: vec![
                        ...
                        OperationDescription::PID(PIDOperation { pid: Pid::new(2., 2.0, 5., 100., 10., 30., 35.), offset: 30 }),
                        OperationDescription::Clip(ClipOperation { min: 30.0, max: 100.0 } ),
                        OperationDescription::Supersample(SupersampleOperation { n: 100 } ),
                        OperationDescription::DampenedOscillator(CriticallyDampenerOperation { m: 0.5, k: 2., dt: 0.25, target: 0.0 }),
                        OperationDescription::DampenedOscillator(CriticallyDampenerOperation { m: 1.0, k: 1., dt: 0.25, target: 0.0 }),
                        OperationDescription::Clip(ClipOperation { min: 30.0, max: 100.0 } ),
                    ],
                    output: Output::PWM,
                    sample_rate: 1000,
                };
            trace!("{}", serde_json::to_string_pretty(&default_pipeline).unwrap());
            default_pipeline
        }
        };
