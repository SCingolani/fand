    ...
    where I: Iterator<Item = f64>,
{
    ...
    fn next(&mut self) -> Option<I::Item> {
        if let Some(val) = self.iter.next() {
            ...
            let output = std::cmp::max(0, std::cmp::min(100, (-control.output as u32))) as f64;
            trace!("PID: {:2.2} -> {:2.2} {:2.2} {:2.2} -> {:2}({:2.2})", val, control.p, control.i, control.d, output, control.output);
            Some(output)
        } else {
            None
        }
    ...
    where I: Iterator<Item = f64>,
{
    ...
    fn next(&mut self) -> Option<I::Item> {
        if let Some(val) = self.iter.next() {
            ...
            let new_vel = 1.0/(1.0 + self.c * fac) * (self.vel * (1.0 - self.c * fac) + fac * (self.acc - acc));
            self.acc = acc;
            self.vel = new_vel;
            self.pos = new_pos;

            trace!("CriticallyDampener: x= {}; v= {}; a= {}", new_pos, new_vel, acc);
            Some(new_pos)
        } else {
            None
        }
    ...
    where I: Iterator<Item = f64>,
{
    ...
    fn next(&mut self) -> Option<I::Item> {
        if self.last_val.is_some() && self.count < self.n {
            trace!("Supersample: Repeated {:?} ({:2})", self.last_val, self.count);
            self.count += 1;
            self.last_val
